...
func TestDcrAddressSerializer(t *testing.T) {
	...
	addr, err := newAddress(fakeWallet, privKey, nil,
		makeBS(0))
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read dcrAddress differ.")
	}
}
...
// 		// private key.
// 		origPubUncompressed := pubkeyFromPrivkey(test.origPrivateKey)
// 		origPubCompressed := pubkeyFromPrivkey(test.origPrivateKey)
...
// 		nextPubCompressedFromPriv := pubkeyFromPrivkey(nextPrivCompressed)
...
// 		// Public keys (used to generate the decred address) MUST match.
...
// 		// the next pubkeys.
// 		pubkeyUncompressed, err := chainec.ParsePubKey(nextPubUncompressedFromPub, chainec.S256())
// 		if err != nil {
// 			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
// 			return
// 		}
// 		pubkeyCompressed, err := chainec.ParsePubKey(nextPubCompressedFromPub, chainec.S256())
// 		if err != nil {
// 			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
// 			return
// 		}
// 		privkeyUncompressed := &chainec.PrivateKey{
// 			PublicKey: *pubkeyUncompressed.ToECDSA(),
// 			D:         new(big.Int).SetBytes(nextPrivUncompressed),
// 		}
// 		privkeyCompressed := &chainec.PrivateKey{
...
// 		if !ok {
// 			t.Errorf("%s: dcrec signature verification failed for next keypair (chained from uncompressed pubkey).",
// 				test.name)
// 			return
// 		}
...
// 		if !ok {
// 			t.Errorf("%s: dcrec signature verification failed for next keypair (chained from compressed pubkey).",
// 				test.name)
// 			return
// 		}
// 	}
...
// 	if !ok {
// 		t.Errorf("dcrec signature verification failed; address's pubkey mismatches the privkey.")
// 		return
// 	}
...
// 	ok = sig.Verify(hash, pubKey)
// 	if !ok {
// 		t.Errorf("dcrec signature verification failed; next address's keypair does not match.")
// 		return
// 	}
...
// 		return
// 	}
// 	pk, _ := chainec.PrivKeyFromBytes(chainec.S256(), make([]byte, 32))
// 	wif, err := dcrutil.NewWIF(pk, tstNetParams, true)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
...
// 	pk := chainec.Secp256k1.NewPrivateKey(chainec.ECTypeSecp256k1)
// 	if err != nil {
...
// 	// import priv key
// 	wif, err := dcrutil.NewWIF((*chainec.PrivateKey)(pk), tstNetParams, false)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
