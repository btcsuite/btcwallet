...
*   **Be Wary of Accidental Capture-by-Reference in Closures**: When launching a goroutine inside a `for` loop, always make a local copy of the loop variable(s).

    ```go
    // WRONG: All goroutines will share the same 'peer' variable.
    for _, peer := range peers {
        go func() {
            connectTo(peer) // BUG: Uses the same peer for all iterations!
        }()
    }

    // RIGHT: Each goroutine gets its own copy.
    for _, peer := range peers {
        peer := peer // Shadow the loop variable.
        go func() {
            connectTo(peer)
        }()
    }
