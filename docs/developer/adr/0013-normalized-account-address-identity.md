...
| Arbitrary account XPub | watch-only | external arbitrary | absent | required |
...
Task 392 introduces it with the existing name and scope inputs plus
`NoChainSync`. Task 343 later adds exact account selection, Task 390 adds custom
scope schema selection, and Task 391 adds the supplied account XPub and optional
fingerprint. Those fields enter the request only with their owning behavior;
the initial request does not publish dormant fields. No separate exact-account,
lnd-specific, or key-family operation is introduced.
...
The retained ordinary numberless-import signature is
...
Caller-declared scope is authoritative only for a path-bound supplied-XPub
request through `NewAccountParams`. Its extended-key version is checked for
