...
func clonePInput(in *psbt.PInput) psbt.PInput {
	out := psbt.PInput{
		...
		PartialSigs:        clonePartialSigs(in.PartialSigs),
		Bip32Derivation:    cloneBip32Derivations(in.Bip32Derivation),
		TaprootBip32Derivation: cloneTaprootBip32Derivations(
			in.TaprootBip32Derivation,
		),
		TaprootScriptSpendSig: cloneTaprootScriptSpendSigs(
			in.TaprootScriptSpendSig,
		),
		TaprootLeafScript: cloneTaprootLeafScripts(
			in.TaprootLeafScript,
		),
	}
...
// clonePartialSigs deep copies a list of ECDSA partial signature records.
func clonePartialSigs(sigs []*psbt.PartialSig) []*psbt.PartialSig {
	if sigs == nil {
		return nil
	}

	clone := make([]*psbt.PartialSig, len(sigs))
	for i, sig := range sigs {
		if sig == nil {
			continue
		}

		clone[i] = &psbt.PartialSig{
...
// cloneTaprootScriptSpendSigs deep copies a list of taproot script spend
// signature records.
func cloneTaprootScriptSpendSigs(
	sigs []*psbt.TaprootScriptSpendSig) []*psbt.TaprootScriptSpendSig {

	if sigs == nil {
		return nil
	}

	clone := make([]*psbt.TaprootScriptSpendSig, len(sigs))
	for i, sig := range sigs {
		if sig == nil {
			continue
		}

		clone[i] = &psbt.TaprootScriptSpendSig{
			XOnlyPubKey: bytes.Clone(sig.XOnlyPubKey),
...
// cloneTaprootLeafScripts deep copies a list of taproot leaf script records.
func cloneTaprootLeafScripts(
	leaves []*psbt.TaprootTapLeafScript) []*psbt.TaprootTapLeafScript {

	if leaves == nil {
		return nil
	}

	clone := make([]*psbt.TaprootTapLeafScript, len(leaves))
	for i, leaf := range leaves {
		if leaf == nil {
			continue
		}

		clone[i] = &psbt.TaprootTapLeafScript{
