...
func AddAllInputScripts(signer *frost.Signer, keys map[string]*btcec.PublicKey, tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []btcutil.Amount, secrets SecretsSource) error {
...
// the input value in the sighash.
func spendTaprootKey(signer *frost.Signer, keys map[string]*btcec.PublicKey, txIn *wire.TxIn, pkScript []byte,
...
func (tx *AuthoredTx) AddAllInputScripts(signer *frost.Signer, keys map[string]*btcec.PublicKey, secrets SecretsSource) error {
