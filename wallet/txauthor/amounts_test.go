...
// TestCheckOutputs verifies that the output-set validator rejects a nil, a
// negative, an individually oversized, and an aggregately oversized output set,
// and returns the checked total for every set it accepts.
func TestCheckOutputs(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := CheckOutputs(tc.outputs)
...
func TestCheckedFeeForSerializeSize(t *testing.T) {
	...
	tests := []struct {
		...
		name: "a fee rounds down to the whole satoshi",
		rate: 1e3,
		size: 1500,
		want: 1500,
	}, {
