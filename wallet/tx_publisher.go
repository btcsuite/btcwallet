...
func (w *Wallet) filterOwnedAddresses(
	...
	map[address.Address]waddrmgr.ManagedAddress, error) {

	ownedAddrs := make(map[address.Address]waddrmgr.ManagedAddress)

	// Pre-deduplicate addresses outside the DB transaction.
	uniqueAddrs := make(map[address.Address]struct{})
