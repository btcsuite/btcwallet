...
var (
	// ErrNotPubKeyAddress is returned when a derived address is not a
	// public key address, and therefore a public key cannot be obtained
	// from it.
	ErrNotPubKeyAddress = errors.New("derived address is not a pubkey " +
		"address")
)
...
type Signer interface {
	...
	// SignMessage signs a message based on the provided intent. The returned
	// Signature is a marker interface that can be asserted to the concrete
	// signature types, ECDSASignature or SchnorrSignature.
	SignMessage(ctx context.Context, path BIP32Path,
		intent *SignMessageIntent) (Signature, error)

	// ComputeUnlockingScript generates the full sigScript and witness required
	// to spend a UTXO. The resulting UnlockingScript struct contains the
	// raw witness and/or sigScript, which can be used to populate the
...
func signAndAssembleScript(params *UnlockingScriptParams,
	privKey *btcec.PrivateKey, scriptInfo *Script) (*UnlockingScript, error) {
...
func (w *Wallet) ComputeRawSig(ctx context.Context, params *RawSigParams) (
	...
	return params.Details.Sign(params, privKey)
}
...
func (w *Wallet) DerivePrivKey(ctx context.Context, path BIP32Path) (
	*btcec.PrivateKey, error) {
	...
	return managedPubKeyAddr.PrivKey()
}
