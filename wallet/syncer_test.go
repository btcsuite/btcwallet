...
func TestStoreScanAddressesIncludesActiveRawImportScope(t *testing.T) {
	...
	s := newSyncer(
		Config{DB: testDBConfig(dbConn), ChainParams: &chainParams},
		mockAddrStore, nil, &mockTxPublisher{}, store, walletID,
	)
	s.legacyStore = kvdb.NewStore(dbConn, nil, mockAddrStore)
