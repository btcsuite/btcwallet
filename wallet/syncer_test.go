...
func TestSyncerRecoveryExcludesNoChainSyncAccounts(t *testing.T) {
	...
	// Arrange: Persist both policies and their addresses so full recovery
	// must filter horizon admission as well as already-materialized watches.
	...
	// Act: Initialize full recovery and the ordinary target separately from
	// the same persisted snapshot, then obtain their observable filter scripts.
	full, err := s.loadFullScanState(t.Context())
	require.NoError(t, err)
	targeted, err := s.loadTargetedScanState(t.Context(), targets)
	require.NoError(t, err)
	...
	// Assert: Each ordinary branch retains its complete lookahead beyond its
	// stored count, while the forbidden account contributes no watched script.
	wantCount := int(2*testScanRecoveryWindow + accounts[0].ExternalKeyCount +
...
func TestSyncerRecoveryRejectsPersistedNoChainSync(t *testing.T) {
	...
	// Arrange: Use the real stored policy and retain the key counts after
	// fixture derivation, so a partial recovery mutation remains observable.
	...
	// Act: Request positive-lookahead recovery for the excluded account.
	state, err := s.loadTargetedScanState(t.Context(), targets)

	// Assert: The wallet-owned identity survives the real SQL lookup, no
	// partial recovery state escapes, and neither stored branch count advances.
	require.ErrorIs(t, err, ErrNoChainSyncRecoveryTarget)
	require.Nil(t, state)
	after, err := s.store.GetAccount(t.Context(), db.GetAccountQuery{
		WalletID:    s.walletID,
	...
	require.Equal(t, before.InternalKeyCount, after.InternalKeyCount)
}
