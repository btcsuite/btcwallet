...
func TestExtractTxAddrs(t *testing.T) {
	...
	require.Len(t, extractedAddrs[2], 0, "OP_RETURN output should have "+
		"no addresses")
...
func TestBroadcastPublishFailsRemoveFails(t *testing.T) {
	...
	err = w.Broadcast(ctx, tx, label)
	require.Error(t, err)
	require.Contains(t, err.Error(), publishErr.Error())
	require.Contains(t, err.Error(), removeErr.Error())
}
