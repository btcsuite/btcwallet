...
// packetSnapshot returns the wire encoding of a packet, so a later call can
// prove the caller's packet was never touched.
//
// The comparison is deliberately made over bytes rather than over the value
// itself: clonePacket is one of the things these tests exist to check, so
// using it to take the snapshot would let a clone that quietly loses a field
// agree with a funding run that quietly loses the same one.
func packetSnapshot(t *testing.T, packet *psbt.Packet) string {
	t.Helper()
...
func TestClonePacketIsIndependent(t *testing.T) {
	...
	// Arrange: fill in every field a caller can set, so the clone is
	// exercised across the whole record rather than the handful of fields
	// the funding path happens to touch.
	packet := testPacket(t)
	...
	// A snapshot of the original, taken through its wire encoding rather
	// than through clonePacket. Using the clone as its own oracle would
	// hide the very bug this test is for: a field copied shallowly would
	// leave the snapshot sharing storage with the original, so a write
	// through the clone would change both and still compare equal.
	before := packetSnapshot(t, packet)

	// Act: clone it, then scribble over every field of the clone.
	clone := clonePacket(packet)
	require.Equal(t, packet, clone)
	require.NotSame(t, packet, clone)
	...
	// Assert: the original is byte for byte what it was, and the clone has
	// genuinely diverged from it.
