...
//
//nolint:maintidx // The parity test covers one complete schema lifecycle.
func testTransactionSchema(t *testing.T, db *sql.DB) {
	...
	_, err := db.ExecContext(ctx, `
		...
		) VALUES (1, 'transactions-test', 1, 1, TRUE, x'01', x'02')
	`)
	...
	minedIDs := make([]int64, 0, 2)
	for i, height := range []int64{100, 101} {
	...
	minedCreditIDs := make([]int64, 0, len(minedIDs))
	...
	minedSpenderID, err := q.InsertTransaction(
		ctx, sqlc.InsertTransactionParams{
			...
			ReceivedUnix:   2000,
			BlockHeight:    sql.NullInt64{Int64: 101, Valid: true},
	...
	require.NoError(t, err)
	require.EqualValues(t, 1, firstCredits[0].IsSpent)
	...
	require.NoError(t, err)
	require.Zero(t, secondCredits[0].IsSpent)
	...
	require.Zero(t, credits[0].IsSpent)
