...
// EnsureSchemaFamily verifies that the SQLite database belongs to this schema
// family, applies the embedded migrations, and records the identity marker. It
// is the production entry point that wraps ApplyMigrations with the
...
// OpenAndMigrate opens a SQLite database and runs the schema-family gate,
...
// schemaBackend adapts a SQLite database handle to the schemaid.Backend
// interface using raw database/sql access.
type schemaBackend struct {
	db *sql.DB
}

// ListTables returns the names of the user tables in the database.
func (b *schemaBackend) ListTables(ctx context.Context) ([]string, error) {
	rows, err := b.db.QueryContext(
		ctx, "SELECT name FROM sqlite_master "+
...
// ApplyMigrations runs the embedded SQLite migration runner.
...
func (b *schemaBackend) InsertMarker(ctx context.Context) error {
	_, err := b.db.ExecContext(
		ctx, "INSERT INTO "+schemaid.IdentityTable+
			" (id, family, generation, created_at) VALUES (1, ?, ?, ?)",
