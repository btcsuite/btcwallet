...
//
// The scenario forces a mid-rollback failure by corrupting a confirmed coinbase
// root hash before the rollback. RollbackToBlock rewinds the wallet sync state
// to the fork point, then fails while marking the corrupted coinbase orphaned.
// Because both effects share one transaction, the wallet's persisted sync tip
// must remain at its original block.
//
...
func TestRollbackToBlockAtomicRollsBackSyncTipOnFailure(t *testing.T) {
	...
	// Corrupt the coinbase root hash so the rollback step fails after the
	// sync-state rewind has already run inside the same transaction.
	corruptTransactionHash(
		t, store, walletID, coinbaseTx.TxHash(), []byte{1, 2, 3},
	)

	// The rollback (boundary = coinbase height) must fail on the corrupted
	// coinbase root after rewinding the sync state to the fork point.
