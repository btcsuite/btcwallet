...
// TestApplyScanBatchResolvesHorizonByName proves the SQL backend resolves a
// scan horizon's owning account by its durable, scope-unique AccountName, not
// by the masked account number. An imported-xpub account stores a NULL
...
// named imported account is the one extended, a derived account still resolves
// by name, and an unknown name fails the batch instead of silently extending
...
func TestApplyScanBatchResolvesHorizonByName(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.Equal(t, uint32(0), derivedInfo.AccountNumber)
	...
	// default derived account's number), so only AccountName resolves the
	// intended target.
	...
	require.NoError(t, err)
	require.Equal(t, db.ImportedAccount, rawImport.Origin)
	...
	// A horizon naming the derived account still resolves by name and extends
	// only the derived account.
	err = store.ApplyScanBatch(t.Context(), db.ScanBatchParams{
		WalletID: walletID,
		Horizons: []db.ScanHorizon{{
			Scope:       scope,
	...
	require.Equal(t, uint32(2), derived.ExternalKeyCount,
		"named derived account must resolve and extend")

	// An unknown account name must fail the batch outright; the fail-safe
	// forbids falling back to the default account 0 on a name miss.
	err = store.ApplyScanBatch(t.Context(), db.ScanBatchParams{
		WalletID: walletID,
		Horizons: []db.ScanHorizon{{
			Scope:       scope,
