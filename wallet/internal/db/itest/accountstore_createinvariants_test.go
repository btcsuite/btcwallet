...
// TestDerivedAccountNumberImmutable verifies that account numbers are
// insert-only because they are structural account identity.
func TestDerivedAccountNumberImmutable(t *testing.T) {
	t.Parallel()
...
func TestCreateDerivedAccountIgnoresImportedAccounts(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, uint32(0), accountNumberNotNil(t, first.AccountNumber))
	...
	require.NoError(t, err)
	require.Equal(t, uint32(1), accountNumberNotNil(t, second.AccountNumber))
}
