...
// TestPostgresExactAccountReopen verifies sparse accounts and key facts survive
// reopening, with lower holes available and the sequential cursor preserved.
func TestPostgresExactAccountReopen(t *testing.T) {
	t.Parallel()

	// Arrange: create sequential and sparse accounts in an isolated database
	// using the production identity and Store initialization path.
	identity, err := db.NewDatabaseIdentity(
		&chaincfg.RegressionNetParams, nil,
	)
	require.NoError(t, err)
	...
	reopened, err := pg.NewStore(t.Context(), cfg)
	require.NoError(t, err)
	t.Cleanup(func() { _ = reopened.Close() })
