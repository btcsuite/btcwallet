...
// consumed without shortening the batch or being reused after reopening.
func TestNewDerivedAddressesSkipsInvalidChildren(t *testing.T) {
	...
	// Act: Fill a batch across the invalid child and reopen before allocating
	// one child across another invalid leaf through the singular wrapper.
	batch, err := store.NewDerivedAddresses(t.Context(), params, 2)
	require.NoError(t, store.Close())
	store = reopen()
	next, nextErr := store.NewDerivedAddress(t.Context(), params)

	// Assert: Both requested children are returned in order, and the invalid
	// indexes remain consumed across both APIs and the new connection pool.
	require.NoError(t, err)
...
func TestNewDerivedAddressesRollsBackFailure(t *testing.T) {
	...
	// Act: Fail the batch, reopen the Store, and read both durable structures
	// before retrying a new request with derivation available again.
	batch, err := store.NewDerivedAddresses(t.Context(), params, 2)
	require.NoError(t, store.Close())
	store = reopen()
	account := getAccountByName(
...
// rows, including concurrent allocators competing for the last normal child.
func TestNewDerivedAddressesExhaustion(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Act: Race two whole batches, join their results, and reopen the
			// Store before requesting an already exhausted branch again.
			for range 2 {
				go func() {
					batch, err := store.NewDerivedAddresses(
						t.Context(), params, tc.count,
			...
			for _, result := range completed {
				if result.err == nil {
					require.Len(t, result.batch, int(tc.count))
					require.Equal(t, db.MaxAddressIndex, result.batch[0].Index)

					winners++
				} else {
					require.ErrorIs(t, result.err, db.ErrMaxAddressIndexReached)
