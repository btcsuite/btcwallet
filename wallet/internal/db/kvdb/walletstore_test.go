...
// newKvdbWalletStoreTestSetup builds a kvdb.Store hooked up to a freshly
// created spendable waddrmgr wallet for the wallet-store master-key tests.
func newKvdbWalletStoreTestSetup(t *testing.T) (*Store, func()) {
	t.Helper()
...
// TestKvdbGetEncryptedHDSeed verifies that GetEncryptedHDSeed returns the
...
func TestKvdbGetEncryptedHDSeed(t *testing.T) {
	t.Parallel()

	store, cleanup := newKvdbWalletStoreTestSetup(t)
	t.Cleanup(cleanup)
