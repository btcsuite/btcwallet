...
func TestValidateCreditAddrMembership(t *testing.T) {
	...
	require.NoError(t, err)
	foreignAddr, err := address.NewAddressPubKey(
	...
	require.NoError(t, err)
	outsiderAddr, err := address.NewAddressPubKey(
	...
	multiSigScript, err := txscript.MultiSigScript(
		[]*address.AddressPubKey{memberAddr, foreignAddr}, 1,
	)
	require.NoError(t, err)
