...
CREATE TABLE transactions (
    ...
    -- For confirmed: block header tx_hash (during sync) or time.Now() (during creation).
    received_timestamp BIGINT NOT NULL CHECK (received_timestamp >= 0),

    -- Serialized transaction data (variable length).
    -- This is the full Bitcoin transaction in wire format.
    serialized_tx BYTEA NOT NULL CHECK (length(serialized_tx) > 0),
