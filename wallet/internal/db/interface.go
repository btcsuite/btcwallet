...
type AddressStore interface {
	...
	// indexes, including valid candidates, without inserting any batch rows.
	// An ambiguous commit must never be retried automatically.
...
type TxStore interface {
	...
	// DeleteUnminedTx removes one unmined transaction branch as a single
	// atomic wallet event. Unlike InvalidateUnminedTx it retains no
	// history.
	//
