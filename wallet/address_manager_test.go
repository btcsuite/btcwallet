...
func TestAllocateNextKeyKvdbDurability(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		// Act: Stop the admitted request loop before closing the database, then
		// reopen it with a fresh Manager so cached counters cannot hide lost
		// writes.
		require.NoError(t, w.Stop(t.Context()))
		require.NoError(t, manager.Close())
		reopened := testKVDBManagerAt(t, dbPath)
		w, err = reopened.Load(LoadWalletParams{
			Name:          params.Name,
