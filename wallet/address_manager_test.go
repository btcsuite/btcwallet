...
func expectStoreNewAddress(t *testing.T, w *Wallet, deps *mockWalletDeps,
	...
	// Ordinary accounts permit receiving, so every allocation first consumes
	// its required policy read before the existing write and notification.
...
func TestNewAddress(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// account's policy read, address write, and notification below.
			...
			// Act: allocate through the receiving API after its account
			// policy has allowed the existing derivation workflow.
...
func TestNewAddressNoChainSyncUnsupported(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Arrange: permit only the account-policy read on a started
			// wallet. Strict mocks reject any derivation, mutation, secret
			// access, or notification attempted after that read.
			w, deps := createStartedWalletWithMocks(t)
			name := "key-only"
			...
			// Assert: the public refusal returns no address, and the sole
			// policy read leaves every receiving side effect uncalled.
...
func TestGetUnusedAddress(t *testing.T) {
	...
	// default account is selected by three unused-address requests; the one
	// fallback allocation below declares its own additional read expectation.
	w, deps := createStartedWalletWithMocks(t)
	...
	// allocation, including its independent account-policy read.
