...
type AddressManager interface {
	...
	// account and address type. This method is the recommended default for
	// obtaining a new receiving address, as it prevents address reuse and
	// avoids creating gaps in the address chain that could impact wallet
	// recovery.
...
//
// GetUnusedAddress returns the earliest unused address for the given account
// and address type. The address is considered "unused" if it has never
// appeared in a transaction. This method is the recommended default for
// obtaining a new receiving address. It prevents address reuse and avoids
// creating gaps in the address chain, which is critical for reliable wallet
// recovery under standards like BIP44 that enforce a gap limit of 20 unused
// addresses. If all previously derived addresses have been used, this method
// will delegate to NewAddress to generate a new one.
func (w *Wallet) GetUnusedAddress(ctx context.Context, account string,
	addrType waddrmgr.AddressType) (btcutil.Address, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// name.
		acctNum, err := manager.LookupAccount(addrmgrNs, account)
		if err != nil {
			return err
		}
	...
	return w.NewAddress(ctx, account, addrType, false)
}
...
func (w *Wallet) ListAddresses(_ context.Context, account string,
	addrType waddrmgr.AddressType) ([]AddressProperty, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		addrBalances := make(map[string]btcutil.Amount)
		...
		for _, u := range unspent {
			_, addrs, _, err := txscript.ExtractPkScriptAddrs(
				u.PkScript, w.chainParams,
			)
			if err != nil {
				return err
			}
			for _, addr := range addrs {
				addrBalances[addr.String()] += u.Amount
			}
		}
		...
		return manager.ForEachAccountAddress(addrmgrNs, acctNum,
			func(maddr waddrmgr.ManagedAddress) error {
				...
				properties = append(properties, AddressProperty{
					Address: addr,
					Balance: addrBalances[addr.String()],
				})
