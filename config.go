...
type config struct {
	...
	Seed       *string `long:"seed" description:"The seed for a new wallet."`
	NoPass     bool    `long:"nopass" description:"If this option is set, the wallet is stored unencrypted. Only used with --create."`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.NoPass && !cfg.Create {
		// Cannot use nopass options except with create.
		err = fmt.Errorf("Cannot use --nopass except with --create")
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if cfg.Seed != nil && !cfg.Create {
		// Cannot use seed options except with create.
		err = fmt.Errorf("Cannot use --seed except with --create")
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
