...
type BitcoindConn struct {
	...
	rpcPollingEvents *BitcoindRPCPollingEvents
...
func NewBitcoindConn(cfg *BitcoindConfig) (*BitcoindConn, error) {
	...
	if cfg.RPCPolling {
		events, err := newBitcoindRPCPollingEvents(
			cfg.PollingConfig, bc.client, bc.sendBlockToClients,
			bc.sendTxToClients,
		)
		if err != nil {
			return nil, err
		}
		bc.rpcPollingEvents = events
		log.Info("HERE")
		return bc, nil
	}

	events, err := newBitcoindZMQEvents(
		cfg.ZMQConfig, bc.sendBlockToClients,
		bc.sendTxToClients,
	)
	if err != nil {
		return nil, err
...
func (c *BitcoindConn) Start() error {
	...
	if c.rpcPollingEvents != nil {
		fmt.Println("HERERERE")
		err := c.rpcPollingEvents.Start()
		if err != nil {
			return err
		}
	}
...
func (c *BitcoindConn) Stop() {
	...
	if c.rpcPollingEvents != nil {
		if err := c.rpcPollingEvents.Stop(); err != nil {
			log.Errorf("error shutting down bitcoind events: %w", err)
		}
	}
	if c.zmqEvents != nil {
		if err := c.zmqEvents.Stop(); err != nil {
			log.Errorf("error shutting down bitcoind events: %w", err)
		}
	}
