...
// AddressIdentifier represents the unique identifier of a voting pool
// address. Given one of these, a bitcoin address can be generated by deriving
// public HD keys from the series' master keys using the branch/index and
// constructing a M-of-N multi-sig script.
type AddressIdentifier interface {
	SeriesID() uint32
	Branch() Branch
	Index() Index
}
...
func (p *Pool) ChangeAddress(seriesID uint32, index Index) (*ChangeAddress, error) {
	...
	addrID := &addressIdentifier{pool: p, seriesID: seriesID, branch: Branch(0), index: index}
...
func (p *Pool) withdrawalAddressFromManagedAddr(seriesID uint32, branch Branch, index Index, mAddr waddrmgr.ManagedScriptAddress) (*WithdrawalAddress, error) {
	...
	addrID := &addressIdentifier{pool: p, seriesID: seriesID, branch: branch, index: index}
...
func (a *addressIdentifier) String() string {
	return fmt.Sprintf("AddressIdentifier seriesID:%d, branch:%d, index:%d", a.seriesID, a.branch,
		a.index)
}

func (a *addressIdentifier) series() *SeriesData {
	return a.pool.Series(a.seriesID)
}

func (a *addressIdentifier) SeriesID() uint32 {
	return a.seriesID
}

func (a *addressIdentifier) Branch() Branch {
	return a.branch
}
