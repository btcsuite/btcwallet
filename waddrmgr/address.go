...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	// The address computation per type is shared with the runtime derivation
	// preparation through deriveAddressFromPubKey, so both paths produce the
	// same legacy address identity.
	newAddress, err := deriveAddressFromPubKey(
		pubKey, compressed, addrType, m.rootManager.chainParams,
	)
	if err != nil {
		return nil, err
	}
