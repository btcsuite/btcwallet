	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			case SatPerVByte:
				...
				require.Equal(t, tc.expectedKVB, r.FeePerKVByte())
				require.Equal(t, tc.expectedKW, r.FeePerKWeight())
				require.Equal(t, tc.expectedSats, btcutil.Amount(r))

			case SatPerKVByte:
				require.Equal(
					t, tc.expectedVB, r.FeePerKWeight().FeePerVByte(),
				)
			...
			case SatPerKWeight:
				require.Equal(t, tc.expectedVB, r.FeePerVByte())
				require.Equal(t, tc.expectedKVB, r.FeePerKVByte())
