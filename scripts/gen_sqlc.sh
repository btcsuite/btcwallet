...
restore_files() {
	...
	for file in walletdb/sqldb/sqlc/migrations/*.up.sql.bak; do
		mv "$file" "${file%.bak}"
	done
}
...
for file in walletdb/sqldb/sqlc/migrations/*.up.sql; do
	echo "Patching $file"
...
go generate walletdb/sqldb/sqlc/generate.go
...
for file in walletdb/sqldb/sqlc/*.sql.go; do
  echo "Patching $file"
