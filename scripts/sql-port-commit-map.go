...
//
//nolint:cyclop,errcheck,forbidigo,funlen,gocognit,goconst,gocyclo,gosec,lll,mnd,noctx,noinlineerr,wsl_v5 // This developer-only generator favors auditable history tables.
package main
...
func readSourceRefs(path string) (map[string]string, error) {
	...
	for _, row := range rows[1:] {
		if len(row) != len(rows[0]) {
			return nil, errors.New("malformed source refs row")
		}
